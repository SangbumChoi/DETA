...
from deta.models.ms_deform_attn import MSDeformAttn
from deta.util.box_ops import box_cxcywh_to_xyxy
from deta.util.misc import _is_tracing, inverse_sigmoid
        ...
        if self.two_stage:
            ...
            if self.assign_first_stage:
                proposal_boxes = box_cxcywh_to_xyxy(
                    enc_outputs_coord_unact.sigmoid().float()
                ).clamp(0, 1)
                device = (
                    proposal_boxes[0].device
                    if torch.jit.is_scripting()
                    else ("cpu" if torch.jit.is_tracing() else proposal_boxes[0].device)
                )
                topk_proposals = []
                for b in range(bs):
                    ...
                    # nms on topk indices
                    if _is_tracing() or len(pre_nms_inds) != len(prop_boxes_b):
                        ...
                        post_nms_inds = batched_nms(
                            prop_boxes_b, prop_logits_b, level_ids, 0.9
                        )
                    ...
                    keep_inds_mask = is_level_ordered & (
                        is_level_ordered.float().cumsum(1) <= q_per_l
                    )  # LS
                    ...
                    if keep_inds_mask.sum() < torch.tensor(
                        topk, device=keep_inds_mask.device
                    ):
