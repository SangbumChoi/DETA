...
import deta.util.misc as utils
import numpy as np
import onnx
import onnxruntime
import torch
import torch.nn.functional as F
from deta.models import build_model
from deta.models.ms_deform_attn import MultiScaleDeformableAttention as MSDeformAttn
...
def get_args_parser():
    ...
    # onnx parameters
    parser.add_argument("--h", default=1344, type=int)
    parser.add_argument("--w", default=1344, type=int)
...
class Etmpy_MultiScaleDeformableAttnFunction(torch.autograd.Function):
    ...
    def symbolic(
        ...
        return g.op(
            "com.microsoft::MultiscaleDeformableAttnPlugin_TRT",
            value,
            value_spatial_shapes,
            value_level_start_index,
            sampling_locations,
...
def MSMHDA_onnx_export(
    ...
    output = output.reshape(int(N), int(Len_q), self.d_model)
...
def compare(args, model):
    x = torch.randn(1, 3, args.h, args.w).cuda()
